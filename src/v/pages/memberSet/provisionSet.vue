...
<script>
	...
	export default {
		...
		methods: {
			...
			getProvisionSet() {
				this.isLoading = true;
				this.$api("memberSet.getProvisionSet").then((result) => {
			...
			save() {
				...
				this.$api("memberSet.saveProvisionSet", {protocol: encodeURIComponent( this.form1.content )}).then((result) => {
